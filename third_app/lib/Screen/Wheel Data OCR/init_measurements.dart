import 'package:flutter/material.dart';
import 'data_manager.dart';
//import 'app_data.dart';

void main() {
  runApp(MyApp());
}

class MyApp extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: InitMeasurements(),
    );
  }
}

class CustomTextWidget extends StatelessWidget {
  final String text;
  final TextStyle textStyle;
  final double letterSpacing;

  const CustomTextWidget({
    Key? key,
    required this.text,
    this.textStyle = const TextStyle(
      fontFamily: 'Open Sans',
      fontSize: 30,
      fontWeight: FontWeight.w400,
      color: Color(0xFFFF8518),
    ),
    this.letterSpacing = 0.0,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Text(
      text,
      style: textStyle.copyWith(letterSpacing: letterSpacing),
    );
  }
}

class InitMeasurements extends StatefulWidget {
  const InitMeasurements({super.key});

  @override
  _InitMeasurementsState createState() => _InitMeasurementsState();
}

class _InitMeasurementsState extends State<InitMeasurements> {
  String selectedOption = 'No';

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        backgroundColor: const Color(0xFF1F1F1F),
        body: SafeArea(
          child: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Padding(
                  padding: EdgeInsets.only(left: 35, top: 75),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      CustomTextWidget(
                        text: 'Initial',
                        letterSpacing: 2.0,
                      ),
                      SizedBox(height: 8),
                      CustomTextWidget(
                        text: 'Measurements',
                        letterSpacing: 2.0,
                      ),
                      SizedBox(height: 16),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'Diameter',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6),
                  child: Row(
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-LHS-Diameter'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-LHS-Diameter'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Left',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                      const SizedBox(
                          width: 16), // Add spacing between the two columns
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-RHS-Diameter'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-RHS-Diameter'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Right',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'Flange Thickness',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6),
                  child: Row(
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager()
                                        .trainData['B-LHS-FlangeThickness'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-LHS-FlangeThickness'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Left',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                      const SizedBox(
                          width: 16), // Add spacing between the two columns
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager()
                                        .trainData['B-RHS-FlangeThickness'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-RHS-FlangeThickness'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Right',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'Flange Width',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6),
                  child: Row(
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-LHS-FlangeWidth'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-LHS-FlangeWidth'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Left',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                      const SizedBox(
                          width: 16), // Add spacing between the two columns
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-RHS-FlangeWidth'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-LHS-FlangeWidth'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Right',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'Flange Gradient',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6),
                  child: Row(
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-LHS-Qr'] = newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager().trainData['B-LHS-Qr'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Left',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                      const SizedBox(
                          width: 16), // Add spacing between the two columns
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager().trainData['B-RHS-Qr'] = newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager().trainData['B-RHS-Qr'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Right',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'Radial Deviation',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6),
                  child: Row(
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager()
                                        .trainData['B-LHS-RadialDeviation'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-LHS-RadialDeviation'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Left',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                      const SizedBox(
                          width: 16), // Add spacing between the two columns
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const SizedBox(height: 16),
                          Container(
                            width: 150,
                            decoration: BoxDecoration(
                              boxShadow: [
                                BoxShadow(
                                  color: Colors.black.withOpacity(0.3),
                                  spreadRadius: 2,
                                  blurRadius: 4,
                                  offset: const Offset(0, 3),
                                ),
                              ],
                            ), // Adjust the width as needed
                            child: TextField(
                              onChanged: (newValue) {
                                DataManager()
                                        .trainData['B-RHS-RadialDeviation'] =
                                    newValue;
                              },
                              style: const TextStyle(
                                color: Color(0xFFF5F5F5),
                              ),
                              controller: TextEditingController(
                                  text: DataManager()
                                          .trainData['B-RHS-RadialDeviation'] ??
                                      ''),
                              decoration: InputDecoration(
                                hintText: 'Right',
                                hintStyle: const TextStyle(color: Colors.grey),
                                filled: true,
                                fillColor: const Color(0xFF313134),
                                border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: BorderSide.none,
                                ),
                                focusedBorder: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(10),
                                  borderSide: const BorderSide(
                                      color: Color(0xFFFF8518), width: 2.0),
                                ),
                              ),
                            ),
                          ),
                        ],
                      ),
                    ],
                  ),
                ),
                const Padding(
                    padding: EdgeInsets.only(left: 35, top: 42),
                    child: Row(
                      children: [
                        CustomTextWidget(
                          text: 'After Cut',
                          textStyle: TextStyle(
                            fontSize: 22,
                            color: Color(0xFFF5F5F5),
                          ),
                        ),
                      ],
                    )),
                Padding(
                  padding: const EdgeInsets.only(left: 35, top: 6, bottom: 30),
                  child: Container(
                    width: 200, // Adjust the width as needed
                    decoration: BoxDecoration(
                      color: const Color(
                          0xFF313134), // Background color of the dropdown button container
                      borderRadius:
                          BorderRadius.circular(10), // Rounded corners
                    ),
                    child: Align(
                      alignment: Alignment
                          .centerRight, // Align the selected text to the right
                      child: DropdownButtonFormField<String>(
                        value: selectedOption,
                        items: const <DropdownMenuItem<String>>[
                          DropdownMenuItem<String>(
                            value: 'Yes',
                            child: Text(
                              'Yes',
                              style: TextStyle(
                                color: Color(0xFFF5F5F5), // Text color
                                fontSize: 16, // Text size
                              ),
                            ),
                          ),
                          DropdownMenuItem<String>(
                            value: 'No',
                            child: Text(
                              'No',
                              style: TextStyle(
                                color: Color(0xFFF5F5F5), // Text color
                                fontSize: 16, // Text size
                              ),
                            ),
                          ),
                        ],
                        onChanged: (String? newValue) {
                          setState(() {
                            selectedOption = newValue ?? selectedOption;
                            bool flag = false;
                            if (selectedOption == "Yes") {
                              flag = true;
                            } else if (selectedOption == "No") {
                              flag = false;
                            }
                            DataManager.afterCut = flag;
                            print(
                                "After Cut flag changed to ${DataManager.afterCut}");
                            // Update trainData based on the selectedOption
                            if (selectedOption == "Yes") {
                              DataManager().appData = DataManager().trainData;

                              DataManager().trainData['A-LHS-Diameter'] = '999';
                              DataManager().trainData['A-LHS-FlangeThickness'] =
                                  '999';
                              DataManager().trainData['A-LHS-FlangeWidth'] =
                                  '999';
                              DataManager().trainData['A-LHS-Qr'] = '999';
                              DataManager().trainData['A-LHS-RadialDeviation'] =
                                  '999';
                              DataManager().trainData['A-RHS-Diameter'] = '999';
                              DataManager().trainData['A-RHS-FlangeThickness'] =
                                  '999';
                              DataManager().trainData['A-RHS-FlangeWidth'] =
                                  '999';
                              DataManager().trainData['A-RHS-Qr'] = '999';
                              DataManager().trainData['A-RHS-RadialDeviation'] =
                                  '999';
                              // print('After Cut is No: ${DataManager().trainData}');
                              // print('App Data: ${DataManager().appData}');
                            } else {
                              // Restore trainData to the original values
                              DataManager().trainData['A-LHS-Diameter'] =
                                  DataManager().appData['A-LHS-Diameter'] ?? '';
                              DataManager().trainData['A-LHS-FlangeThickness'] =
                                  DataManager()
                                          .appData['A-LHS-FlangeThickness'] ??
                                      '';
                              DataManager().trainData['A-LHS-FlangeWidth'] =
                                  DataManager().appData['A-LHS-FlangeWidth'] ??
                                      '';
                              DataManager().trainData['A-LHS-Qr'] =
                                  DataManager().appData['A-LHS-Qr'] ?? '';
                              DataManager().trainData['A-LHS-RadialDeviation'] =
                                  DataManager()
                                          .appData['A-LHS-RadialDeviation'] ??
                                      '';
                              DataManager().trainData['A-RHS-Diameter'] =
                                  DataManager().appData['A-RHS-Diameter'] ?? '';
                              DataManager().trainData['A-RHS-FlangeThickness'] =
                                  DataManager()
                                          .appData['A-RHS-FlangeThickness'] ??
                                      '';
                              DataManager().trainData['A-RHS-FlangeWidth'] =
                                  DataManager().appData['A-RHS-FlangeWidth'] ??
                                      '';
                              DataManager().trainData['A-RHS-Qr'] =
                                  DataManager().appData['A-RHS-Qr'] ?? '999';
                              DataManager().trainData['A-RHS-RadialDeviation'] =
                                  DataManager()
                                          .appData['A-RHS-RadialDeviation'] ??
                                      '';
                              // print('After Cut is No: ${DataManager().trainData}');
                              // print('App Data: ${DataManager().appData}');
                            }
                          });
                        },
                        decoration: InputDecoration(
                          filled: true,
                          fillColor: const Color(
                              0xFF313134), // Background color of the dropdown button
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(10),
                            borderSide: BorderSide.none,
                          ),
                          focusedBorder: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(10),
                            borderSide: const BorderSide(
                                color: Color(0xFFFF8518), width: 2.0),
                          ),
                        ),
                        elevation:
                            1, // Set the elevation to change the background color of the dropdown
                        icon: const Icon(
                          Icons.arrow_drop_down,
                          color:
                              Color(0xFFF5F5F5), // Color of the dropdown arrow
                        ),
                        dropdownColor: const Color(
                            0xFF313134), // Background color of the dropdown list
                      ),
                    ),
                  ),
                )
              ],
            ),
          ),
        ),
      ),
    );
  }
}
